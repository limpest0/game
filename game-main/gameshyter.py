...
class Bonus(pygame.sprite.Sprite):
    ...
    def update(self):
        self.rect.x -= 3  # Задаем фиксированную скорость
...
def show_title_screen():
    ...
    if play_button_visible:
        ...
        play_button_surface.set_alpha(play_button_alpha)
...
def reset_game():
    global enemy_speed, kill_count, last_spawn_time, arrows, enemies, bonuses, game_over, show_title, previous_kill_count, shoot_delay, bonus_effect_end_time
    enemy_speed = 1
    kill_count = 0
    ...
    game_over = False
    show_title = True
    shoot_delay = 1000  # Восстанавливаем стандартную задержку между выстрелами
    bonus_effect_end_time = 0  # Сбрасываем эффект бонуса
    show_title_screen()
...
def spawn_bonus():
    ...
    new_bonus = Bonus(width, y_position, hit_image_path, hit_image_path)
...
last_bonus_time = 0  # Время последнего спавна бонуса
bonus_spawn_interval = 20000  # Интервал между спавнами бонусов (20 секунд)
bonus_effect_end_time = 0  # Время окончания эффекта бонуса
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and show_title:
            ...
            # Проверка на нажатие кнопки Play
    ...
    if current_time - last_bonus_time > bonus_spawn_interval:
        spawn_bonus()
    ...
    if not game_over and not show_title:  # Добавлено условие для игрового экрана
        ...
        if collision_enemy:
            for arrow in collision_enemy:  # each bullet
                # each alien that collides with that bullet
        ...
        if collision_bonuses:
            shoot_delay = 0  # Убираем задержку между выстрелами
            # Устанавливаем время окончания эффекта (15 секунд)
            bonus_effect_end_time = current_time + 15000

        if current_time > bonus_effect_end_time:
            shoot_delay = 1000  # Восстанавливаем задержку между выстрелами
    ...
    if game_over:
        if current_time < show_title_end_time:
            # Перерисовываем кнопку, чтобы она была чёрной
